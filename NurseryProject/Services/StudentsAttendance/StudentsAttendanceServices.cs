    ...
    public class StudentsAttendanceServices
    {
        public List<StudentsAttendanceDto> GetAll()
        {
            using (var dbContext = new almohandes_DbEntities())
            {
                var date1 = dbContext.StudentsAttendances.Where(x => x.IsDeleted == false && x.Student.IsDeleted == false).OrderBy(x => x.CreatedOn)
                    .Select(x => new {x.Id, x.Date, x.ClassId, x.Class.Name, Plase = x.Class.StudyPlace.Name, Type = x.Class.Level.StudyType.Name ,Level=x.Class.Level.Name,Class=x.StudyClass.Name,Year=x.StudyClass.StudyYear.Name}).ToList();
                ...
                if (date1.Count()>0)
                {
                    ...
                    {
                        var model = dbContext.StudentsAttendances.Where(x => x.IsDeleted == false && x.Student.IsDeleted == false && x.Date == item.Date && x.ClassId == item.ClassId).ToList();
                        ...
                        mod.Add(new StudentsAttendanceDto { Id = item.Id, StudyPlaceName = item.Plase, StudyTypeName = item.Type, LevelName = item.Level, StudyYearName = item.Year, StudyClassName = item.Class, NumAllAttend = model.Count().ToString(), NumAttend = model2.Count().ToString(), ClassName = item.Name, Date = item.Date.Value.ToString("yyyy-MM-dd") });
                    };
                }
        ...
        public ResultDto<Models.StudentsAttendance> Create(Models.StudentsAttendance model, List<Guid> IsAttend, Guid UserId)
        {
            using (var dbContext = new almohandes_DbEntities())
            {
                ...
                var stId = dbContext.Levels.Where(x => x.IsDeleted == false && x.Id == class1.LevelId).FirstOrDefault().StudyTypeId;
                var model1 = dbContext.StudentsClasses.Where(x => x.IsDeleted == false && x.ClassId == model.ClassId && x.Class.Level.StudyTypeId == stId).ToList();
                foreach (var item2 in model1)
                {
                    var test = dbContext.StudentsAttendances.Where(x => x.IsDeleted == false && x.StudentId == item2.StudentId && x.ClassId == model.ClassId && x.Class.Level.StudyTypeId == stId && x.Date == model.Date).FirstOrDefault();
                    if (test == null)
                    {
        ...
        public ResultDto<Models.StudentsAttendance> Edit(Models.StudentsAttendance model, Guid UserId)
        {
            using (var dbContext = new almohandes_DbEntities())
            {
                var result = new ResultDto<Models.StudentsAttendance>();
                var Oldmodel = dbContext.StudentsAttendances.Find(model.Id);
